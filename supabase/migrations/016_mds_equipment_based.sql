-- ============================================
-- Migration 016: MDS Berbasis Nama Equipment + Sequence + Generate Bulanan
-- ============================================

-- 1. Tambah kolom sequence_no pada preventive_maintenance
ALTER TABLE preventive_maintenance
ADD COLUMN IF NOT EXISTS sequence_no INT DEFAULT 0;

-- 2. Tambah index agar lookup mds_templates by name lebih cepat
CREATE INDEX IF NOT EXISTS idx_mds_templates_name ON mds_templates(name);

-- 3. Fungsi Generate WO untuk seluruh bulan tertentu
      ...
      IF pm.plan_start <= v_check_date THEN
        ...
        IF v_remainder = 0 THEN
          ...
          IF NOT EXISTS (
            ...
            -- Update next_due ke tanggal WO terakhir yang dibuat
...
-- 4. Grant akses RPC function ke authenticated users
GRANT EXECUTE ON FUNCTION generate_monthly_pm_work_orders(INT, INT) TO authenticated;
GRANT EXECUTE ON FUNCTION generate_monthly_pm_work_orders(INT, INT) TO anon;

-- 5. Update jadwal pg_cron — generate bulanan tiap awal bulan jam 00:05
DO $$
BEGIN
  PERFORM cron.unschedule('generate-pm-wo-monthly');
EXCEPTION WHEN OTHERS THEN
  -- ignore jika belum ada
END $$;

SELECT cron.schedule('generate-pm-wo-monthly', '5 0 1 * *', 'SELECT generate_monthly_pm_work_orders()');
